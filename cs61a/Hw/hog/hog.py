...
def tail_points(opponent_score):
    ...
    ones = mod(opponent_score,10)
    tens = mod((opponent_score - ones)/10,10)
    # print(ones,tens)
    result = int(2*abs(tens-ones)+1)
    return result
    # END PROBLEM 2
...
def take_turn(num_rolls, opponent_score, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    if num_rolls == 0:
        result = tail_points(opponent_score)
    else:
...
def play(strategy0, strategy1, update,
    ...
    # initialization
    current_score=[score0,score1]
    dice_num = [0,0]
    strategy = [strategy0,strategy1]
    Dice = dice

    while(True):
        dice_num[who] = strategy[who](current_score[who],current_score[1-who])
        current_score[who] = update(dice_num[who],player_score=current_score[who],\
            opponent_score=current_score[1-who],dice = Dice)
        if if_win(current_score[who]):
            break
        else:
            who = 1 - who
    
    score0 = current_score[0]
    score1 = current_score[1]
...
def always_roll(n):
    ...
    # BEGIN PROBLEM 6
    "*** YOUR CODE HERE ***"
    def strategy(self_score,opponent_score):
        return n
    return strategy
    # END PROBLEM 6
...
def is_always_roll(strategy, goal=GOAL):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    last_result = strategy(0,0)
    if_always = True
    for self_s in range(0,goal):
        for oppo_s in range(0,goal): 
            new_result = strategy(self_s,oppo_s)
